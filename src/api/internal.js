...
const api = axios.create({
  baseURL: "https://aitool-8j7h.onrender.com",
...
export const signup1 = async (data) => {
  let response;
...
export const submitBlog = async (data) => {
  ...
  try {
    response = await api.post("/addtools", data);
  } catch (error) {
    return error;
  }
...
export const submittools = async (data) => {
  let response;

  try {
    response = await api.post("/addformdata", data);
  } catch (error) {
    return error;
  }
